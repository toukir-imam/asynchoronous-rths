function [ step,avgArray,avgGatArray,avgMoveTimeArray,population,adults,bestAgent ] = asyncEvolutionLRTAStarDiffFood( maps,problem,initialPopulation,maxAllowedPopulation,energyMultiplier,difficutyLevel,eraLength )
...
%Author : Toukir Imam (mdtoukir@ualberta.ca)
    ...
    for i =1:initialPopulation
        ...
        agents{i} = createOffspring5(id,problem,maps,p1Gene,p2Gene,noMutation,geneMin,geneMax,energyMultiplier);
    ...
    while ~isempty(agents) 
        ...
        if ~isempty(lastagents)
            bestAgent= findBestAgent(lastagents,bestAgent,problem,'LRTAStarDifficulty');
        ...
        for aI =1:length(agents)
            if ((agents{aI}.goal.x == agents{aI}.x) && (agents{aI}.goal.y == agents{aI}.y))
                ...
                newBorn{numNewBorn} = createOffspring5(id,problem,maps,agents{aI}.gene,p2Gene,mutationRate1,geneMin,geneMax,energyMultiplier);
                numNewBorn = numNewBorn+1;
                id = id+1;
                newBorn{numNewBorn} = createOffspring5(id,problem,maps,agents{aI}.gene,p2Gene,mutationRate2,geneMin,geneMax,energyMultiplier);
                numNewBorn = numNewBorn+1;
                ...
                %create duplicate
                p2Gene=NaN;
                agents{aI} = createOffspring5(agents{aI}.id,problem,maps,agents{aI}.gene,p2Gene,noMutation,geneMin,geneMax,energyMultiplier);
