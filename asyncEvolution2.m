function [ bestAgent ,step,expanse ,stat,statBest] = asyncEvolution2( maps,problem,attr,features )
...
agents = repmat(dummyAgent,[1,maxAllowedPopulation*3]);
...
bestAgent.byAStarSubopt = dummyAgent;

%% stat
%stat = repmat(dummyAgent,[20,initialPopulation]);
%statBest = repmat(bestAgent,[1,20]);
...
if features.tracing
    for i =1:attr.numTraceGene
        agents(i) = createOffspring6(i,problem,maps,attr.traceGene,noMutation,geneMin,geneMax,energyMultiplier,attr);
        fprintf('trace gene born with energy %5.3e to solve OTC %5.3e\n',agents(i).energy,agents(i).hsS0);
        agents(i).isTraceGene = true;
    end
end
...
while numPopulation ~=0
    ...
    %end
    %store the population gene
    %if mod(step,200) ==0
    %    k =1;

    %        for i = 1:length(agents)
    %            if ~agents(i).isDummy
    %                stat(eStep,k) = agents(i);
    %                k = k+1;
    %                statBest(1,eStep) = bestAgent;
    %            end
    %        end
    %        eStep =eStep +1
    %        if eStep >10
