...
%mutationRate = 0.01;
mutationRate = 2*ones(1,length(geneMax));
...
while (simI < numSims)
    ...
    while (toc(tt) < maxTime && ~isempty(agent))
        ...
        for i = 1:length(deads)
            dead_generation = [dead_generation deads{i}.generation];
            dead_weight = [dead_weight deads{i}.gene(3)];
        end
        agent = agent(~dead);
        popChange = any(dead);
        ...
        for aI = 1:length(agent)
            ...
            if (a.x == a.goal.x && a.y == a.goal.y)
                ...
                subopt_2_w_plot=[subopt_2_w_plot a.gene(3)];
                a.numTrials = a.numTrials+1;
                parents_plot_w = [parents_plot_w a.gene(3)];
                ...
                subplot(1,3,1)
                s = plot(generation_plot,weightH_plot,'bo');
                S = plot(subopt_g_plot,parents_plot_w,'go')
                s = plot(dead_generation, dead_weight ,'rx');
                xlabel('generation')
                ylabel('weight H')
                subplot(1,3,2)
                ...
                subplot(1,3,3)
                s = plot(subopt_2_w_plot,subopt_plot,'bo');
                xlabel('Weight H')
                ylabel('suboptimality')
                hold on
