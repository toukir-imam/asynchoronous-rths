function [ step, minArray,maxArray,avgArray,avgGatArray,avgMoveTimeArray,population,adults,bestGene ] = asyncEvolutionMultiOffspringSex( maps,problem,initialPopulation,maxAllowedPopulation,baseEnergy,energyMultiplier,difficutyLevel,eraLength)
    ...
    while ~isempty(agents) 
        ...
        for aI =1:length(agents)
            if ((agents{aI}.goal.x == agents{aI}.x) && (agents{aI}.goal.y == agents{aI}.y))
                ...
                newBorn{numNewBorn} = createOffspringSexual(id,problem,maps,agents{aI}.gene,p2Gene,mutationRate1,geneMin,geneMax,energy);
                numNewBorn = numNewBorn+1;
                id = id+1;
                newBorn{numNewBorn} = createOffspringSexual(id,problem,maps,agents{aI}.gene,p2Gene,mutationRate2,geneMin,geneMax,energy);
                numNewBorn = numNewBorn+1;
                %old style
